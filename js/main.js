...
var burger = document.querySelector('.burger'),
    ...
    buttonHidden = document.querySelector('.card__btn'),
    divCardsHidden = document.querySelector('.promo__cards_hidden'),
...
buttonHidden.addEventListener('click', function () {
  divCardsHidden.classList.remove('promo__cards_hidden');
  buttonHidden.classList.add('card__btn_hidden');
});
...
var tabs = document.querySelector('.tabs__header'),
...
function fTabs(event) {
  if (event.target.className == "tabs__item") {
    var currentDataTab = event.target.dataset.tab;
    ...
    for (var _i = 0; _i < tabsInfo.length; _i++) {
      if (currentDataTab == _i) {
        tabsInfo[_i].classList.add('active');
      } else {
        tabsInfo[_i].classList.remove('active');
      }
    }
  }
}
